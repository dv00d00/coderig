...
{
    // Multi-match cap. `show` prints whole declarations, so an unqualified pattern matching dozens of symbols
    // would flood the terminal; the matches are still DISCLOSED (the "showing N of M" footer + the ambiguity
    // notice), matching how `symbols` caps its listing.
    ...
    private sealed record Options(string Pattern, int Context, int Limit, string? Format, bool Time);
    ...
    private static async Task<int> RunAsync(Options opts, CommandIo io)
    {
        ...
        // Same disclosure the traversal commands give: a substring pattern can span several unrelated
        // symbols, and `show` renders ALL of them (up to --limit) rather than silently picking one.
        AmbiguityNotice.WarnIfAmbiguous(
            io.TextOutput.Error,
            opts.Pattern,
            FactPathFinder.DistinctMatchTargets(matches.Select(m => m.SymbolId), opts.Pattern)
        );
