...
// ---- file effect lens ----------------------------------------------------------------------------------
// The lens moved to filelens.js — it grew a filter surface, a findings vocabulary (HazardKinds tiers 1-3),
// a minimap and a navigation model, none of which the tree view shares. main.js imports `FileEffectsView`
// from there directly rather than re-exporting through here, so the dependency edge stays one-way
// (filelens -> components, for `baseName` / `shortLabel` / `highlightCSharp`) with no import cycle.
