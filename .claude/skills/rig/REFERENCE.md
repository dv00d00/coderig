...
| `rig annotate <file> [--summary] [--method pat] [--from n] [--to n] [--limit n] [--format tsv] [--store ref] [--only tok] [--exclude tok] [--min-depth n] [--max-depth n] [--direct] [--looped] [--no-dispatch] [--host url] [--cold]` | The **file lens**: one indexed file's source with the effect families each METHOD and each CALL LINE reaches. Badges are `family!` at depth 0 (the effect is in that body) and `family:N` for the nearest one N calls away; a method's badge is the shallowest distance over everything it reaches. `--summary` prints only the per-method table; `--method` renders each matching declaration span as a window; `--from/--to` render an arbitrary window. **Line precision only** — extraction mined no column, so several calls on one line share that line's badges (stated in the footer). A site with an empty target is an effect at a call into external code (no in-solution callee to name). `families:` in the header is every family ASKED about, not those found. Source attribution is `rig show`'s contract (working tree only when attributable, else the indexed revision `(from git <sha>)`). An ambiguous `<file>` substring REFUSES and lists candidates. Under `--format tsv`: `method`/`site` rows (always file-wide) then `src` rows (windowed, text last). Rendering goes through `FileEffectLens`, the same projection the web `/api/file-effects` view uses, so CLI and browser cannot disagree. Badge grammar is `family` + distance (`!` here / `:N` away) + `*` (the effect is inside a LOOP here — the amplification tier, only ever on a `!` badge) + `?` (that reach exists only through a dispatch hop with several candidate implementations — a lead, not a fact; a one-implementation hop is deterministic and unflagged). Effects inside lambdas FOLD onto the declaring method, so the method depth does not count the invisible hop into the lambda and can read one lower than `rig reaches` for the same effect. **Every filter flag is PROJECTION-side**: they narrow the already-computed badges, so any combination is free, a surviving badge's number is identical to its unfiltered value, and an active filter always prints a `FILTERED: …` header line so a narrowed view cannot be mistaken for a quiet file. `--only`/`--exclude` accept a provider or family token; a provider token WIDENS to its family (family-grain lens — the widening and the siblings it also matched are printed on stderr) and an unknown token matches nothing. `--intrinsic`/`--async` are deliberately unavailable: both change the closure, not the view. |
...
- **`rig index` builds internally — no external pre-build.** One bare `rig index <sln>` call runs a
  ...
  clone, restore once or pass `--restore`. `index` is the sole extraction command. **Prefer the full solution**: `index --from <csproj>` (the entry-scoped
