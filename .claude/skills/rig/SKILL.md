...
rig amplify [--min-degree n] [--top n] [--format tsv]   # NON-LINEAR discovery: effects ranked by AMPLIFICATION DEGREE (stacked loop contexts). deg>=2 = super-linear; separate recursion + actor:tell sections
...
rig watch Sln.slnx --once --query "reaches Program.Main"  # boot, answer once, exit
rig serve [--port 5050]                      # store-backed interactive web explorer
```
...
- **Amplification is a SECOND finding tier, ON by default** (2026-08). `derive` gets an `Amplification (looped effects — structural inventory)` section AFTER Hazards; `tree --view hazards` marks it inline with `🔁` (hazards keep `⚠`); tsv row type **`amplification`** — *not* `hazard`. `looped_effect` used to be a bare count line in "Observations on effects"; it is now displayed and broken down by **`provider:operation`**, provider-agnostic — a looped `http:POST` is as visible as a looped `llblgen:read`. **Why this can be on-by-default while `n_plus_1` needed FP calibration: `looped_effect` is a structural FACT** (the effect is lexically inside an iteration context, soundly, no guess), whereas `n_plus_1` is a JUDGMENT about whether the key varies. It is still **NOT a hazard** — `HazardKinds.IsHazard("looped_effect")` stays false, so every `hazard` row, hazard count and `--expect-*` gate is byte-unchanged. **`--no-amplification`** (on `derive`/`tree`/`impact`) reproduces the old output exactly. **The displayed scope is RULES DATA** (`observations.amplification`, builtin-rules.json), staged to **network-crossing** providers where ×N means N round trips: `http`, `llblgen` (read/write/delete/bulk_write/fetch), `db_command`/`db_connection`/`db_reader`/`efcore`, `object_store` (read/write), `queue`, `actor` (tell/ask/spawn). Local providers (`shared_state`, `entity_cache`, `lock`, `io`, `permission`, `audit`, `parallel`, the intrinsics) are OUT of the default and **stay counted in the generic Observations block** — nothing is hidden, and widening is a one-line rules edit. MedDBase: **704** in-scope sites of 1,531 looped non-intrinsic (827 remain a generic count; `--intrinsic` adds 12,873 looped `alloc` + 8 looped `throw`, both out of scope).
- **`impact` reports the amplification DELTA, TERSE**: `ep_amplification_added` / `ep_amplification_removed` `<kind> <route> <provider> <operation> <sites>` — **one row per (EP × provider:operation)**, never per site. It is the only signal for "a loop was introduced around an existing call": the effect SET is unchanged there, so no `ep_effect_*` row can show it, while the cost went ×N. A pure site-count move (3→4) is deliberately NOT a delta.
- **Cross-method amplification is an opt-in THIRD tier.** When merged rules contain `crossMethodAmplification`, `derive` correlates a looped anchor call with a gated I/O effect beneath it and emits `cross_method_amplification` rows at anchor × witness grain plus an anchor-grain human summary. The web hazards surface folds those anchors into its mark stream; `?crossMethod=false` opts out. This is a reachability correlation and review candidate, not proof of runtime N+1.
- **`rig amplify` is the FOURTH tier — DEGREE, the only one that composes.** Tiers 2/3 are per-loop: `looped_effect` says "this effect is in a loop", `cross_method_amplification` correlates exactly ONE loop anchor with an effect beneath it. Neither can say ×N vs ×N². `amplify` chains the (already FP-calibrated) `FactIterationFanoutDeriver` anchors into loop CHAINS over the dispatch-aware traversal, so caller-loop × callee-loop composes across call edges, virtual dispatch and lambdas. **degree ≥2 = super-linear**; `-1`/`recursion` = the chain enters a call cycle (unbounded — no finite number is honest). Three sections: super-linear, fire-and-forget (`actor:tell`, queueing not round trips), recursion. Confidence **✔** = all contributing loops are cross-method (independent by construction); **~** = includes intra-method line-span-containment nesting, which is a HEURISTIC (the facts carry no loop-nesting depth, only the innermost loop per call site). **Sibling loops over one collection are additive and correctly stay degree 1** — disjoint spans never compose (only 240 of 1,290 MedDBase multi-loop methods actually nest; a per-method loop count overstates 81% of them). **Degree is an UPPER BOUND over composable contexts, not proof one execution stacks them all — reach is path-insensitive; confirm a specific chain with `rig path` before quoting it.** `--top` is PER SECTION, so a capped run is not a recall measurement (use `--min-degree 1 --top 5000` for that). MedDBase: 1m06s over all 10,109 EPs — 663 super-linear + 186 recursion.
...
## Live working-tree queries
`rig watch <sln|slnx>` cold-loads a retained workspace, watches saved `.cs` edits, and keeps facts in
memory; it writes no immutable store. One resident host owns a working directory by default. `--no-serve`
permits a deliberate second stdin-only host, `--once` exits after boot, `--query "…"` answers once, and
`--restore` is only for an unrestored tree.
...
`dead` is currently an explaining disabled stub because its former all-hops dispatch traversal is unsound
against the one-hop forward engine. Use `callers <method> --roots` only as an approximation.
...
- **`impact` DOES see predicate-only changes, via a separate signal** (shipped 2026-07-27). A guard tightened around an unchanged effect (audit suppressed behind a new flag, permission narrowed) still yields **zero** effect delta — the call site is unchanged — so `--expect-no-effect-change` PASSES. What catches it is `guard_condition_delta` rows + the **`--expect-no-guard-narrowing`** gate: per call edge whose gating condition moved, a `narrowed` / `widened` / `changed` verdict with the base→head conditions and the effects reachable from the callee. `narrowed` = fires on strictly FEWER paths, the audit-suppression shape. Keyed on the EDGE (not the effect, whose own guard set is usually empty — the condition is an ancestor edge's). Classification is SYNTACTIC conjunct containment, robust to reformatting/comments but an over-approximation: it recognises "AND another clause on" and falls back to `changed`. `--only`/`--exclude` scope these rows too. **Needs both stores re-indexed by a rig ≥ 2026-07-27** (guards on lambda edges + polarity).
...
- **Guards were MISSING on lambda and method-group edges** — `Call(() => …)`/`Call(Helper.Do)` carried none (0 of 65,450 lambda edges on MedDBase), so every effect inside a lambda created in an `if` read as **must-run**: rig claiming "always happens" for conditional work. **Fixed 2026-07-27 in extraction; RE-INDEX to pick it up.** Two consequences worth remembering: the **must-run spine is over-inclusive on any older store**, and **the guard sits on the EDGE, not the effect** — an empty guard set on an effect does NOT mean it always fires, since the gating condition can be several frames up. This (not the call-site model) is why `impact` saw no delta for MR !11025.
...
- **Seed resolution is SHARED and now DISCLOSES four outcomes** (fixed 2026-07-27; the reported `tree`-vs-`reaches` divergence was a stale-`LATEST` store mix-up, not a resolver bug): (1) no match → `No symbol matches` + exit 1 on `tree`/`callers`/`reaches`/`path` alike (`path` names which endpoint); (2) matched leaf → exit 0 plus a stderr `note: resolved to X; it makes no in-solution calls`, so a real 0 never reads as "does nothing"; (3) a `P:`property/`F:`field/`E:`event → told it is not a call-graph node, use the accessor (`get_X`); (4) ambiguity → the multiplicity note. An overloaded param-free FQN correctly spans ALL overloads.
- **`callers --entrypoints` == 0 does NOT mean unreachable — and it now tells you WHY.** A zero answer reports the FRONTIER: the reverse-reachable methods with no in-solution caller, where the chain tops out, with file:line. Those are the BOUNDARY (template/Dom interpolation, reflection, DI-by-name, an external caller), not proof of dead code. A target nothing calls at all gets a distinct message ("the chain is empty, not cut short"). Lambdas are NOT the gap — rig models them fine (`~λ0` nodes appear in the chain); attributing a frontier to lambdas was a wrong conclusion this disclosure exists to prevent.
