...
delivery edges (tags come from the ruleset's `deliveryRules` — `event_raise` builtin, `actor_tell` in the
MedDBase ruleset) join a raise to its subscribers by SYMBOL/name only — no
...
(`event_cycle`) is unaffected — it always considers every delivery edge whose mechanism the ruleset marked
`cycleDelivery` (core names no delivery tag of its own; a ruleset declaring none gets no cycles).
