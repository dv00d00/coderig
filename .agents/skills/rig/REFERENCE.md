...
| `rig watch <sln\|slnx> [--rules p…] [--once] [--query text] [--no-serve] [--restore]` | Cold-load and retain a workspace, apply saved `.cs` edits to in-memory facts, and serve synchronous live `reaches`/`path`/`callers`/`tree` queries. Routed `--async` / `--include-delivery` requests decline to the immutable-store fallback until the demand graph can synthesize delivery sites exactly. Writes no store. `--once` boots then exits; `--query` answers once; `--no-serve` permits a second stdin-only host; `--restore` is opt-in for an unrestored tree. |
