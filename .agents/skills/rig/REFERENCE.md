...
| Command | Purpose |
|---|---|
| `rig index <sln\|csproj> [--rules p…] [--identity id] [--from entry.csproj] [--framework tfm] [--parallelism n] [--merge] [--include-tests] [--no-graph] [--restore]` | Extract facts. Solution = all projects' source in one run (callgraph crosses boundaries in-process). Single `.csproj` = that project's source only (refs become metadata DLLs). **`--from entry.csproj`** = index only the entry project's transitive ProjectReference closure (minus test projects) — still ONE cross-project workspace; writes the closure to `relevant-projects.json`. **`--framework tfm`** selects that TFM for multi-targeted projects; without it the first declared TFM is used. **`--parallelism n`** parallelises the (independent, no-binary) design-time builds. **Save is fast + crash-safe by default**: durability-off pragmas + write-to-temp + atomic rename over `rig.db` (so a fresh `index` cleanly REPLACES — the old "index APPENDS" footgun is gone; that now only applies to `--identity`). **`--include-tests`** keeps test projects (excluded by default); **`--no-build-cache`** / **`--verify-build-cache`** control the design-time-build cache. Cache misses are staged but become reusable only after that exact project payload passes a zero-error base Roslyn compilation; failed/no-compilation projects are evicted, so a disclosed partial run cannot poison the next run. **`--restore`** runs the MSBuild `Restore` target before each design-time build — **OFF by default**: `/restore` walks every project's ProjectReference closure, which was ~80% of the build phase on MedDBase (322.9s → 57.4s without it, identical symbols/refs). rig indexes a tree someone already built; pass `--restore` only for an unrestored checkout. (There is NO `--durable`/journaled-write option — removed.) |
| `rig runs` | List runs + symbol/reference/di counts (provenance / health check). |
| `rig derive [--rules p…] [--limit n] [--only p,…] [--exclude p,…] [--exclude-namespace pfx…] [--no-gate] [--no-amplification] [--list-providers] [--format tsv]` | Stage-2 over facts: re-derive effects + page/action/background/wcf entry points + **classified** handoffs (background/timer/actor/event, by dispatcher + registration site, with an `async_handoff` note) + promoted handoff origins; unclassified-methodGroup residual collapsed to a count. One DB open, one rule load. **`--list-providers`** prints the valid `--only`/`--exclude` token set (an unknown token WARNS on stderr, no longer silent). **`--exclude-namespace <prefix>`** (repeatable) drops hazard findings whose enclosing namespace starts with the prefix (hazards only; effects unaffected). Hazard rollup is deduped per `(type, method)` with a `×N` site count; header reads "N site(s) across M method(s)". |
...
| `rig watch <sln\|slnx> [--rules p…] [--once] [--query text] [--no-serve] [--restore]` | Cold-load and retain a workspace, apply saved `.cs` edits to in-memory facts, and serve live `reaches`/`path`/`callers`/`tree` queries in sync, `--async`, and `--async --include-delivery` modes. Async delivery is projected from keyed event-symbol/configured endpoint partitions without flattening the resident graph or event-site corpus; unsupported rules/caps fail closed to the routed immutable-store fallback. Writes no store. `--once` boots then exits; `--query` answers once (traversal flags are trailing and quote-aware); `--no-serve` permits a second stdin-only host; `--restore` is opt-in for an unrestored tree. |
...
| `rig symbols <pat> [--kind <k>] [--limit n] [--no-lambdas] [--format tsv\|json]` | Declared symbols (method/type/property/field/event). `--no-lambdas` drops compiler `~λ`/`<>c` noise; TSV has a named `id kind name signature file line assembly` schema and JSON is a camel-case envelope with `shown`/`total`/`truncated`. Human truncation stays human; machine stdout stays parseable. |
| `rig show <pat> [--context n] [--all [--limit n]]` | Source text of one unambiguous declaration (`Line`..`EndLine`), with a line-number gutter. Ambiguity fails closed, leaves stdout empty, and prints exact rerunnable FQN candidates on stderr; `--all` is the explicit opt-in to render matches and `--limit` caps that path (default 5). **Attribution is enforced:** the working tree is read only when the store is clean, `SourceCommit` == HEAD, and that file is unmodified; otherwise the indexed revision is read from git and marked `(from git <sha>)`. If neither is possible it prints `file:line` plus a one-line reason rather than lines it cannot attribute. Declarations over 400 lines are truncated with an explicit marker. |
...
All traversal depth options prefer the readable spelling `--max-depth`; `--maxdepth` and `--depth` remain
accepted aliases. Every immutable-store query writes one provenance line to stderr after its schema gate:
`current`, `STALE` (HEAD/worktree moved), `UNVERIFIABLE` (indexed dirty), or `freshness unknown`. Explicit
`--store` selections are marked `pinned`; default selections identify `LATEST`. Machine stdout is unchanged.
`rig runs` is already the complete all-store provenance report and suppresses this generic line.
...
|---|---|---|
| `--rules` | `index`, `watch`, `derive`, `reaches`, `tree`, `callers`, `path`, `impact`, `entrypoints`, `dispatch-fans`, `hotspots` | `serve`, `symbols`, `refs`, `files`, `runs`, `show`, `di`, `profile` |
| `--store` | every read command | `runs` (enumerates ALL stores by design), `profile` |
| `--format` | `derive`, `reaches`, `tree`, `callers`, `path`, `impact`, `entrypoints`, `effects-diff`, `hotspots`, `symbols`, `show` | `watch`, `serve`, `refs`, `files`, `runs`, `di`, `profile` |
...
Known gap (not a bug, but worth knowing): `symbols` now has TSV/JSON, but the other fact readers
`refs`/`files`/`di` remain text-only. Use `symbols --format tsv|json` for scriptable symbol discovery;
parse the others' human output only when no derivable query can answer the question.
...
- **Actor / message-passing boundaries are rule-bounded.** Configured argument-addressed producer/registration
  pairs (for example Echo.Process `.tell` + `.spawn`) become exact or disclosed fan-out delivery edges under
  async traversal. Dynamic routers, reflection-only handlers, unconfigured mechanisms, and request/reply
  correlation remain uncrossable; absence beyond such a boundary is not proof that the runtime cannot reach it.
...
- **`rig index` builds internally — no external pre-build.** One bare `rig index <sln>` call runs a
  ...
  `--reuse-build-cache` are tuning knobs, not required); the old "MSBuild first" step is obsolete. It does
  NOT **restore**, though — `--restore` is off by default, so packages must already be restored; on a fresh
  clone, restore once or pass `--restore`. A bare unrestored run may still publish an explicitly disclosed
  partial store, but its erroring/no-compilation project outputs are NOT admitted to `.rig/dtb-cache`; one
  subsequent `--restore` rebuilds those misses and admits only the projects that compile cleanly. No
  `--no-build-cache` recovery step is needed. `index` is the sole extraction command. **Prefer the full solution**: `index --from <csproj>` (the entry-scoped
