...
- Settle the open rendering question below: should the line whose call carries the dispatch be marked too?

## Open product question, undecided: dispatch-only badge, silent line

This card is mainly an invariant to assert. This one item is different in kind — an undecided rendering
choice, not a correctness assertion — so it is called out separately rather than mixed into the work above.

When a method badge exists ONLY because of dispatch, should the line whose call carries the dispatch also be
marked? Silence is honest — no call is proven — but it leaves a reader hunting for which call it was. The
mirror case is already understood: after the dispatch-disclosure fix `ImageEdit.Save` reads `echo:2?`,
disclosed as dispatch-only, with no line carrying it because no markable call proved it.
