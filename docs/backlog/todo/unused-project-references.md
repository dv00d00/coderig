...
**Status:** TODO / MEDDBASE-CALIBRATION (inputs AVAILABLE — see the un-park note below) — slice 1 (CLI) and slice 3 (web view) shipped 2026-07-08. The remaining
...
`LATEST`); source at `c:/git/meddbase-main-application` (`9f83dab5b7`). **PARTIALLY unblocked:** the project tree, source and store
are all present, so the static half of the reflection/markup risk ranking is runnable now. What is still
out of reach is the **AUT-backed acceptance** — the automated UI suite lives in neither repo, and it is what
both engineers leaned on to confirm a cut was safe. So: build and rank the risk, but do not claim a
verified-safe cut list without it. Runtime-loaded and package-reference analysis remain independent follow-ons.
