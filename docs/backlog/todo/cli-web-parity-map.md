...
2. [The behavioral-EP count differed by one](../done/cli-web-parity-2-impact-behavioral-count.md) — shipped via
   the shared Impact selection view; historical row attribution is now a separate measurement card.
3. [`/api/meta` `derivationVersion` lacked store identity](../done/cli-web-parity-3-api-meta-derivation-version-lacks-store-identity.md)
...
- The historical behavioral count differed by one row. Shared selection/count semantics shipped; only the
  unavailable row's provenance remains on [impact-extra-row-attribution](./impact-extra-row-attribution.md).
...
- ~~Child 1's §2 fork.~~ **DECIDED 2026-09-02 and revised 2026-09-03:** match the CLI — intrinsics hidden by
  ...
  [the terminal Impact filter record](../done/impact-web-effect-filters-client-side.md).
  **Still open for the five endpoints where `intrinsic` IS a server parameter** folded into
  ...
  IndexedDB.
- The historical extra EP's intrinsic-only versus hazard-only attribution is still unestablished; it now lives
  on [its measurement card](./impact-extra-row-attribution.md). Selection/count correctness is closed.
