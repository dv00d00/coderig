...
appear in `BuildTimes` and become warmable. Putting the cost INTO `BuildTimes` matters as much as the saving:
nothing currently instruments this derivation, so no measurement shows it and the background warmer cannot
warm what it cannot see.

### The open sub-question this ticket owns — what the memo keys on
