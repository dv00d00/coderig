...
> Caveat: `RiderFileEffectTransport` is NOT plugin-only - `WatchCommand.cs:709-781` and `LiveQueryServer.cs:350-478` use it, so parking the plugin does not make that transport dead code.
