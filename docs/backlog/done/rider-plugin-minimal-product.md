...
- Add a lazy witness-path interaction rather than transporting paths for every method — the contract and
  acceptance live in [file-lens-grain-5-lazy-witness-path](../todo/file-lens-grain-5-lazy-witness-path.md).
...
- [Explicit solution-to-host association](../needs-review/rider-explicit-solution-to-host-association.md) — both discovery bullets, host and restart.
- [Project/compilation identity for linked and multi-target files](../needs-review/rider-project-compilation-identity.md)
- [Automated Rider SDK daemon test for Code Vision and gutter registration](../needs-review/rider-sdk-daemon-test-for-code-vision-and-gutter.md)
- [Render `Looped` plus tier-1/2/3 findings in the designed budget](../needs-review/rider-render-looped-and-tier-findings.md)
- [Move the backend source out of `experiments/`](../needs-review/rider-backend-out-of-experiments.md)

The lazy witness-path interaction keeps its existing cross-link —
[file-lens-grain-5-lazy-witness-path](../todo/file-lens-grain-5-lazy-witness-path.md) — and deliberately gets
no second card.
