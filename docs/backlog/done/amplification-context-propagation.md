...
4. Per-request recompute of the correlation on `/api/hazards` → [cross-method-hazards-cache](../todo/derivation-cache-2-cross-method-hazards-cache.md).
...
- [CLI `tree` has no amplification fold](../needs-review/cli-tree-has-no-amplification-fold.md) — "What remains" item 1.
- [FP class 3 — memoized / loop-invariant receiver](../needs-review/amplification-memoized-loop-invariant-receiver.md) — item 2.
- [FP — `Option<A>` binds through `System.Linq.Enumerable`](../needs-review/amplification-option-as-ienumerable-false-positive.md) — item 3.

Item 4 (per-request recompute of the correlation on `/api/hazards`) already had a card and keeps it:
