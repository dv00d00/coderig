...
[flaky-clientpage-proxy-extraction](../needs-review/flaky-clientpage-proxy-extraction.md).
...
[bounded-reach-inputs-drop-enclosing-scopes](./bounded-reach-inputs-drop-enclosing-scopes-shipped.md).
...
1. **[A rules edit does not reach the baked graph](../todo/question-vs-plan-1-baked-call-edges-ignore-rules-edits.md)** — HIGH, and
...
2. **[Redirect rules applied asymmetrically](../todo/question-vs-plan-3-redirect-rules-applied-asymmetrically-across-graph-paths.md)**
...
[path-disclosures-computed-off-the-loaded-subgraph](../todo/question-vs-plan-2-path-disclosures-computed-off-the-loaded-subgraph.md).
...
Filed CRITICAL as
[live-index-serves-confident-answers-from-a-broken-compilation](./live-index-serves-confident-answers-from-a-broken-compilation-shipped.md),
...
[Live EP derivation is per-QUERY, not per-generation](../todo/derivation-cache-3-live-ep-derivation-is-per-query-not-per-generation.md):
...
- [Final gate and the disposable-clone release workout](../todo/live-index-final-gate-and-disposable-clone-workout.md)
  — the "Next:" named in the status line above.
- [Memory growth across a long-lived host](../needs-review/live-index-memory-growth-across-a-long-session.md)
  — "Open, ranked" #2 plus the interner's open many-edits growth curve.
- [No add-document path, so a new file is silently omitted from a routed answer](../needs-review/live-index-has-no-add-document-path-for-new-files.md)
  — "Still undisclosed on a routed answer" #1.
- [Generated documents are not re-extracted per file](../needs-review/live-index-generated-documents-are-not-re-extracted.md)
  — "Still undisclosed on a routed answer" #2.
- [Duplicate `dispatch_edges` and type-relation rows in the cold store](../needs-review/duplicate-dispatch-edge-and-type-relation-rows.md)
  — "Open, ranked" #6; unrelated to this program, found by it.

Linked rather than restated:

- the per-line `~compile-error` chip and the unobserved generated-document diagnostics —
  [failed-compilation-disclosure](../todo/failed-compilation-disclosure.md).
- the long-standing ClientPage flake lead —
  [flaky-clientpage-proxy-extraction](../needs-review/flaky-clientpage-proxy-extraction.md).
- "Two `rig watch` in one directory" was DECIDED on 2026-08-22 (refuse to boot), so it needs no card.
