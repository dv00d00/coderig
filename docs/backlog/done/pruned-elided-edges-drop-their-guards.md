...
- [`TreeCommand`'s root filter still asks the old question](../needs-review/pruned-elided-1-tree-root-filter-asks-the-old-question.md)
- [Root ORDER is load-bearing in the pretty renderer](../needs-review/pruned-elided-2-root-order-is-load-bearing.md)
- [The rule lives twice — a `reachesEffect` flag on `TreeNodeDto`](../needs-review/pruned-elided-3-reaches-effect-flag-on-the-dto.md)

All three are held in `needs-review/` rather than scheduled: none is a regression (each is same-as-before
behaviour), so their value is a judgment call, not a defect to fix.
